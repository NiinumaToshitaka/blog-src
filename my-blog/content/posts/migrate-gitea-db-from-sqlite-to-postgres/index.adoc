...
この記事では，Giteaの運用を開始したあとに，データベースをデフォルトのSQLiteからPostgreSQLに移行する手順を説明します。

== 注意

Giteaの運用を開始してからデータベースを移行することは可能ではありますが，できることなら最初からPostgreSQLを使用するべきです。
...
以下のコマンド footnote:[<https://docs.gitea.io/en-us/backup-and-restore/#using-docker-dump>] で移行元のSQLiteのデータをダンプします。
このとき，データベースのデータをPostgreSQLの構文でダンプすることで，PostgreSQLにインポートできるようにします。

なお，このコマンドを実行するときは，移行元のGiteaのDockerコンテナが実行中である必要があります。

[source, bash, subs="attributes+"]
----
# PostgreSQLのSQL構文でダンプ
docker exec -it \
    -u git \
    -w /tmp \
...
``/data/gitea/conf/app.ini``は``[database]``セクションを以下のとおり書き換えます。
